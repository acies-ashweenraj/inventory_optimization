...
def common_schedule_func(df, echelon_type):
    ...
    schedule = []   # ✅ local list, not global
    ...
    for i in range(len(ss_df)):
        ...
        year  = int(ss_df.loc[i, "Year"])
        month = int(ss_df.loc[i, "Month"])
        SKU   = int(ss_df.loc[i,"ItemStat_Item"])
        ...
        for j in range(no_of_orders):
            order_date = first_order_date + timedelta(days=j * cycle)
            schedule.append({
                "From": parent_name,
                "Echelon": echelon_name,
                "Year": year,
    ...
    schedule_df = pd.DataFrame(schedule).drop_duplicates()
