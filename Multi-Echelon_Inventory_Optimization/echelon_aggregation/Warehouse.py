...
def warehouse_data(df):
    ...
    #     ordering_cost = ORDERING_COST[CODE_MAP[echelon_df.loc[i,"Warehouse"]]]
    #     holding_cost = HOLDING_COST[CODE_MAP[echelon_df.loc[i,"Warehouse"]]]
    #     lead_time = LEAD_TIME[CODE_MAP[echelon_df.loc[i,"Warehouse"]]]


    #     # echelon_df.loc[i,"monthly_eoq"]=operations.eoq_manual(ordering_cost,holding_cost,echelon_df.loc[i,"Warehouse_Monthly_Demand"])
    #     echelon_df.loc[i,"monthly_eoq"]=operations.EOQ(ordering_cost,holding_cost,echelon_df.loc[i,"Warehouse_Monthly_Demand"])

    #     echelon_df.loc[i,'cycle_time']=operations.cycle_time(echelon_df.loc[i,'monthly_eoq'],echelon_df.loc[i,'Warehouse_Monthly_Demand'])
    ...
    #     echelon_df.loc[i,"reorder_point"]=operations.reorder_point(echelon_df.loc[i,"Warehouse_Monthly_Demand"],echelon_df.loc[i,"effective_lead_time"])

    #     echelon_df["DC"] = echelon_df["DC"].fillna(0).astype(float).astype(int)
